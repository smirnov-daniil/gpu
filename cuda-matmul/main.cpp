...
static int read_matrices_padded(
  ...
  unsigned int* n_pad, unsigned int* k_pad, unsigned int* m_pad)
{
  unsigned int nkm[3];
  ...
  *m_pad = ((*m + (TILE_M * VEC_Y) - 1) / (TILE_M * VEC_Y)) * (TILE_M * VEC_Y);
  *k_pad = ((*k + TILE_K - 1) / TILE_K) * TILE_K;
  *n_pad = ((*n + (TILE_N * VEC_X) - 1) / (TILE_N * VEC_X)) * (TILE_N * VEC_X);
...
int main(int argc, char* argv[]) {
  ...
  std::string_view input, output, device_index = "-1", realization = "0";
  ...
  if ((program == 1 || program == 2)
    ? read_matrices_padded(fin, &host_a, &host_b, &host_c, &n, &k, &m, &n_pad, &k_pad, &m_pad)
  ...
  case 0:
    ...
    break;
  case 1:
    gpu_tile_matrix_mul_padded(device, host_a, host_b, host_c, n_pad, k_pad, m_pad);
    break;
  case 2:
    gpu_vector_matrix_mul_padded(device, host_a, host_b, host_c, n_pad, k_pad, m_pad);
    break;
  case 3:
    gpu_tile_matrix_mul(device, host_a, host_b, host_c, n, k, m);
    break;
  case 4:
    gpu_vector_matrix_mul(device, host_a, host_b, host_c, n, k, m);
    break;
  }
